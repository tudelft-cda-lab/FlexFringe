/**
 * @file string_probability_estimator_v2.h
...
#ifndef __STRING_PROBABILITY_ESTIMATOR_V2__
#define __STRING_PROBABILITY_ESTIMATOR_V2__
...
class string_probability_estimator_v2_data: public evaluation_data {
    friend class string_probability_estimator_v2;
protected:

    REGISTER_DEC_DATATYPE(string_probability_estimator_v2_data);
...
public:
    string_probability_estimator_v2_data() : evaluation_data::evaluation_data(){        
        final_prob = 0;
    }
    virtual void print_transition_label(std::iostream& output, int symbol) override;
    virtual void print_state_label(std::iostream& output) override;
    ...
    void update_probabilities(const double prior) noexcept {
        for(int i=0; i<symbol_probability_map.size();++i){
            normalized_symbol_probability_map[i] = symbol_probability_map[i] + prior;
        }
    }
...
class string_probability_estimator_v2: public evaluation_function {

protected:
    REGISTER_DEC_TYPE(string_probability_estimator_v2);
...
public:
    ...
    static std::vector<double> normalize_probabilities(string_probability_estimator_v2_data* data, const double remaining_mass, const int alph_size);
