...
void type_state_comparator_data::print_transition_label(std::iostream& output, int symbol){
    //output << "TODO" << endl;
};

void type_state_comparator_data::print_state_label(std::iostream& output){
    output << "TODO" << std::endl;
};

void type_state_comparator_data::print_transition_label_json(std::iostream& output, int symbol){
    //output << "TODO" << endl;
};

void type_state_comparator_data::print_state_label_json(std::iostream& output){
    output << "TODO" << std::endl;
};
...
void type_state_comparator_data::add_tail(tail* t){
    ...
    if(node_type!=-1 && type != node_type) 
        throw std::logic_error("Input is not learnable. Found inconsisten types for the same path");
