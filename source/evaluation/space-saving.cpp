...
double space_saving_data::get_upper_bound(space_saving_data* other){
    ...
    for(int i=0; i<R; ++i){
        for(int j=0; j<R; ++j){
            ...
            similarities.push_back( std::make_tuple(mu_hat, i, j) );
        }
    }
    ...
    if(bounds.size() == 0){
        throw new std::runtime_error("Runtime-error: The size of the bounds object should not be zero.");
    }
...
void space_saving_data::undo(evaluation_data *right) {
    ...
    throw std::runtime_error("Undo not possible with space saving heuristic.");
...
void space_saving_data::print_state_label(std::iostream& output){
...
void space_saving::initialize_before_adding_traces() {
    ...
    if(MERGE_WHEN_TESTING){
        ...
        std::cout << "WARNING: testmerge option should not be turned on with this heuristic and has been turned off automatically. The nature of the sketches prohibits undo-operations." << std::endl;
    }

    if(EPSILON <= double(0)){
       throw new std::runtime_error("Epsilon parameter must be larger than 0.");
    }

    if(R == 0){
        std::cout << "WARNING: R parameter chosen to be zero. No bootstrap will be performed, and this can result in bad and/or wrong results." << std::endl;
    }
}
