...
void membership_state_comparator_data::print_state_label(std::iostream& output){
    if(DEBUG_REPS){
        std::stringstream ss;
        std::ofstream fh;
        ...
        for(const std::vector<float>& vec: traverse_vectors){
            ss = std::stringstream();
        ...
        fh << "final\n";
        for(const std::vector<float>& vec: final_vectors){
            ss = std::stringstream();
            for(const auto x: vec)
                ss << x << " ";
            ss << '\n';
...
void membership_state_comparator_data::update_sums(const std::vector<float>& internal_rep){
    ...
    if(LS.size()==0){
        ...
        if(LS.size() != SS.size()){
            std::cerr << "Error in evaluation function. LS and SS should have same length but have " << LS.size() << " and " << SS.size() << std::endl;
            throw std::exception();
        }

        // initializing here saves us resizing later on
        means = std::vector<float>(internal_rep.size());
        std_devs = std::vector<float>(internal_rep.size());
...
float membership_state_comparator::get_diff(const std::vector<float>& left_v, const std::vector<float>& right_v) const {
