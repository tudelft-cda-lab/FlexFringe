...
css_data::css_data() : alergia_data::alergia_data() {
    ...
    if(!symbols_initialized){
        for(int i = 0; i < NSTEPS_SKETCHES; ++i){
            seen_symbols.push_back( std::set<int>() );
        }
 ...
 */
const std::vector<int> css_data::minhash_set(const std::set<int>  shingle_set, const int n_gram_size) const{
    ...
    std::vector<int> res;
    const int NUMBER_OF_HASHES = std::min(n_gram_size, MINHASH_SIZE);
 ...
 */
const std::vector<int> css_data::encode_sets(const std::vector< std::set<int> > shingle_sets) const {
    std::vector<int> res;

    std::vector< std::vector<int> > hashed_sets;
    for(int i = 0; i < shingle_sets.size(); ++i){
        const std::set<int> shingle_set = shingle_sets[i];
 ...
 */
const std::vector<int> css_data::get_symbols_as_list(tail* t) const {
    ...
    if(!CONDITIONAL_PROB){
        ...
        std::vector< std::set<int> > shingles;
        std::set<int> current_shingle;
        for(int i = 0; i < NSTEPS_SKETCHES; ++i){
            if(t == 0){
                i = NSTEPS_SKETCHES; // kill the inner loop
                break;
            ...
            const int symbol = t->get_symbol();
            if(symbol == -1){ // termination symbol via flexfringe-convention
                shingles.push_back(std::set<int>{-1}); // code cannot be negative, hence no clash possible 
                break;
            }
            ...
            shingles.push_back(std::set<int>(current_shingle));
...
void css_data::print_state_label(std::iostream& output){
