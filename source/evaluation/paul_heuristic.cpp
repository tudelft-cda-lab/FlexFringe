...
void paul_data::print_state_label(iostream& output){
    count_data::print_state_label(output);
    output << "\nc : " << to_string(lm_confidence) << " | nm : " << to_string(num_merged) << "\n";

    for(auto c: all_confidences){
        output << to_string(c.first) << " | " << to_string(c.second) << "\n";
    }
};
...
bool paul_heuristic::overlap_consistent(apta_node* left, apta_node* right) const {
    ...
    r1 = max(r1, float(0.01));
    r2 = max(r2, float(0.01));
...
double paul_heuristic::compute_score(state_merger *merger, apta_node* left, apta_node* right){
    ...
    return double(n_real_real_pairs)/*  + 0.1 * n_real_inferred_pairs + 0.05 * n_inferred_inferred_pairs */;
};
