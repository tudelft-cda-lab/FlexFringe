...
void string_probability_estimator_v2_data::print_transition_label(iostream& output, int symbol){
    output << symbol << ": " << symbol_probability_map[symbol];
};

void string_probability_estimator_v2_data::print_state_label(iostream& output){
 ...
 * @param node The node.
 */
vector<double> string_probability_estimator_v2::normalize_probabilities(string_probability_estimator_v2_data* data, const double remaining_mass, const int alph_size) {
    ...
    vector<double> estimated_masses(data->get_outgoing_distribution());
