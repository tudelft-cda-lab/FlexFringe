 ...
 * @brief With a loop.
 */
refinement* evidence_based_strategy::perform(refinement_set* possible_refs, std::shared_ptr<node_to_refinement_map_T> node_to_ref_map) {
  refinement* top_ref = *(possible_refs->begin());
  apta_node* red_node = top_ref->red;
  ...
  std::unordered_set< refinement* >& refs_under_consideration = node_to_ref_map->at(red_node); // it's ok to modify this list at this stage
  if(blue_node != nullptr){
    for(refinement* ref: node_to_ref_map->at(blue_node)) refs_under_consideration.insert(ref);
  }

  std::unordered_set<int> updated_nodes;
