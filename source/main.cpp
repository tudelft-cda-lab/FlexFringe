...
void run() {
    ...
    if(OPERATION_MODE == "batch" || OPERATION_MODE == "greedy") {
        ...
        daemonize();
        std::thread inputThread(listen_for_input, fifo_path);
        ...
        while(!TERMINATED) {
            if (STREAM_BATCH_INPUT_PATH.empty()) {
                ...
                if (fitness_only) {
                    ...
                    std::vector<std::vector<tuple<int, int>>> state_sequences = stream_obj.get_state_sequences(traces, merger);

                    std::vector<double> fitnesses = EA_utils::compute_fitness_values(state_sequences, merger->get_aut()->get_root(), FITNESS_TYPE);
                    ...
                    // to make sure fitness values are not rounded off when converted to string before writing to file
                    fitness_file << std::fixed << std::setprecision(6); 
                    ...
                    for (auto state_sequence : state_sequences) {
                        int root_state = get<0>(state_sequence[0]);
                        state_sequence_file << root_state;
                        for (int i = 1; i < state_sequence.size(); i++) {
                            tuple<int, int> t = state_sequence[i];
                            state_sequence_file << " " << get<0>(t);
                        }
                        state_sequence_file << "\n";
                    }
