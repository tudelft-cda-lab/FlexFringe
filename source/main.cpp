...
void run() {
    ...
    if(OPERATION_MODE == "active_learning"){
        std::cout << "Run in active learning mode." << std::endl;
        auto al = active_learning_main_func();
        al.run_active_learning();
        std::cout << "Finished running, terminating program." << std::endl;
        return;
    }
    ...
    if(OPERATION_MODE == "batch" || OPERATION_MODE == "greedy") {
        ...
        // TODO: Is the streaming mode currently broken?
        ...
        std::cout << "subgraphextraction mode selected" << std::endl;
