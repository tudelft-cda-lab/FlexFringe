...
void regex_builder::initialize(apta& the_apta, state_merger& merger, std::tuple<bool, bool, bool>& coloring, const std::function<char(int)>& mapping_func){
    ...
    for (apta_node* n : states) {
        ...
        try {
            ...
            throw std::runtime_error("Could not get predicted type, did you set --predicttype 1?");
        }
        if (type.size() == 0) {
            throw std::runtime_error("Could not get predicted type, did you set --predicttype 1?");
        }
...
std::string regex_builder::to_regex(std::vector<apta_node*> final_nodes) {
    ...
    auto final_node = std::make_unique<apta_node>();
    for (auto* n : final_nodes) {
        my_transitions[n].insert(std::make_pair(final_node.get(), ""));
    }
    auto start_node = std::make_unique<apta_node>();
    ...
    std::cout << "Got regex with size: " << regex.size() << std::endl;
