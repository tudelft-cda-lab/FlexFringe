 ...
 * @return int 
 */
const int dfa_conflict_search_base::get_teacher_response(const std::vector<int>& substr, const std::unique_ptr<oracle_base>& oracle, inputdata& id) const {
  return oracle->ask_sul(substr, id).GET_INT();
}
 ...
 */
int dfa_conflict_search_base::parse_dfa(const std::vector<int>& seq, apta& hypothesis, inputdata& id){
