...
namespace dfa_conflict_search_namespace {
  ...
  class linear_state_query_conflict_search : public linear_conflict_search_base {
    protected:
      const int get_sul_response(const std::vector<int>& substr, const std::unique_ptr<sul_base>& sul, inputdata& id) const override;
