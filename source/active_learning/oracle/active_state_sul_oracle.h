...
class active_state_sul_oracle : public active_sul_oracle {
  protected:
    int get_sul_response(const std::vector<int>& query_string, inputdata& id) const override;
  ...
  public:
    active_state_sul_oracle(std::shared_ptr<sul_base>& sul) : active_sul_oracle(sul) {
        ...
        cex_search_strategy = std::unique_ptr<search_base>(new bfs_strategy(MAX_AL_SEARCH_DEPTH));
