...
class sqldb_sul_random_oracle : public eq_oracle_base {
  protected:
  ...
  public:
    explicit sqldb_sul_random_oracle(std::shared_ptr<sul_base>& sul) : eq_oracle_base(sul) {
        my_sqldb_sul = dynamic_pointer_cast<sqldb_sul>(sul);
        if (my_sqldb_sul == nullptr) {
            throw logic_error("sqldb_sul_random_oracle only works with sqldb_sul.");
        }
    };
