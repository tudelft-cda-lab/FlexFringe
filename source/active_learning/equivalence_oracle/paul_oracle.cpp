 ...
 */
int paul_oracle::get_teacher_response(const vector< vector<int> >& query_string, const std::unique_ptr<base_teacher>& teacher, inputdata& id) const {
    vector< pair<int, float> > res = teacher->ask_type_confidence_batch(query_string, id);
...
std::optional<pair<vector<int>, int>> paul_oracle::equivalence_query(state_merger* merger,
    ...
    while (query_string_opt != nullopt) { // nullopt == search exhausted
        ...
        int inferred_value = get_teacher_response(query_string, teacher, id);
        ...
        if (inferred_value != pred_val) {
            ...
            pair< vector<int>, optional<response_wrapper> > conflict_rep_pair = conflict_searcher->get_conflict_string(query_string[0], hypothesis, teacher, id);
