...
class sul_factory {
  ...
  public:
     ...
     */
    static std::vector< std::shared_ptr<sul_base> > create_suls(const sul_key&& key){ return create_suls(); }
};
 ...
 */
template<typename T>
std::unique_ptr<algorithm_base> oracle_factory::create_algorithm_obj(T&& oracle){
  if(ACTIVE_LEARNING_ALGORITHM=="ldot")
    throw invalid_argument("Not implemented yet."); //return make_unique<ldot>(std::forward<T> oracle);
  else if(ACTIVE_LEARNING_ALGORITHM=="lsharp")
    return make_unique<lsharp>(std::forward<T> oracle);
  else if(ACTIVE_LEARNING_ALGORITHM=="lstar_imat")
    return make_unique<lstar_imat>(std::forward<T> oracle);
  else if(ACTIVE_LEARNING_ALGORITHM=="lstar")
    return make_unique<lstar>(std::forward<T> oracle);
  else if(ACTIVE_LEARNING_ALGORITHM=="paul")
    return make_unique<paul>(std::forward<T> oracle);
  else if(ACTIVE_LEARNING_ALGORITHM=="probabilistic_lsharp")
    return make_unique<probabilistic_lsharp>(std::forward<T> oracle);
  else if(ACTIVE_LEARNING_ALGORITHM=="weighted_lsharp")
    return make_unique<weighted_lsharp>(std::forward<T> oracle);
  else
    throw invalid_argument("The active learning algorithm name has not been recognized by the algorithm object factory.");
}
