 ...
 */
void overlap_fill_batch_wise::complement_nodes(vector< vector<int> >& query_traces, vector< pair<apta_node*, int> >& query_node_symbol_pairs, unordered_set<apta_node*>& seen_nodes, std::unique_ptr<apta>& aut, std::unique_ptr<base_teacher>& teacher, apta_node* left, apta_node* right, const int depth){
