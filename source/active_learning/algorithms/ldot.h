...
class ldot_algorithm : public algorithm_base {
  private:
    ...
    /** External structures */
    std::unique_ptr<evaluation_function> my_eval;
    std::unique_ptr<apta> my_apta;
    std::unique_ptr<state_merger> my_merger;
    std::shared_ptr<sqldb_sul> my_sul;
    std::unique_ptr<sqldb_sul_random_oracle> random_oracle;
