 ...
 *
 */
void lsharp_algorithm::complete_state(unique_ptr<state_merger>& merger, apta_node* n, inputdata& id,
    ...
    while (ENSEMBLE_RUNS > 0 && n_runs <= ENSEMBLE_RUNS) {
        ...
        for (blue_state_iterator b_it = blue_state_iterator(the_apta->get_root()); *b_it != nullptr; ++b_it) {
            ...
            if (blue_node->get_size() == 0)
                continue; // This should never happen: TODO: Delete line

            // this is a difference with Vandraager (they only complete red nodes), but we need it for statistical
            // methods
            complete_state(merger, blue_node, id,
                           alphabet); // TODO: do we need this one here really? I can't see it at the moment why
            ...
            //   refinement* best_merge = extract_best_merge(&possible_refs); // implicit assumption: prefers merges
            //   over extends best_merge->doref(merger.get()); performed_refinements.push_back(best_merge);
            // }
            if (possible_refs.size() == 0) {
            ...
            /* {
              ...
              if(model_nr % 10 == 0) print_current_automaton(merger.get(), "model.", to_string(model_nr) +
            ".new_fringe"); // printing the final model each time
              ++model_nr;
            } */
        }
        ...
        if (ENSEMBLE_RUNS > 0 && n_runs == ENSEMBLE_RUNS) {
            ...
            for (auto top_ref : performed_refinements) { top_ref->doref(merger.get()); }
