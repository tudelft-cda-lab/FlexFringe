...
struct sf_tree_node {
  ...
  public:
    ...
    bool is_final() noexcept {return terminated;}
};

class suffix_tree {
  private:
    sf_tree_node* root;
    std::list<sf_tree_node> node_store; // making sure we do not lose them
  ...
  public:
    ...
    std::optional< std::vector<int> > next() const noexcept;
