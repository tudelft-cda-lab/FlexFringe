...
# the following will be set by load_nn_model()
#init_dict = None
...
def do_query(seq: list):
  ...
  print("string: ", list(query_string.size()), query_string.dtype)
  mask = make_tensor_causal_masks(query_string)
  print("mask: ", list(mask.size()), mask.dtype)
  with torch.no_grad():
    output = model(input_ids=query_string, attention_mask=mask, return_dict=True, output_attentions=False)
  print("output: ", output)
  logits = torch.squeeze(output.logits).numpy()
  print("logits: ", logits)

  logits = logits[last_token].tolist()
  print("last_token: ", last_token, ", list: ", logits)
