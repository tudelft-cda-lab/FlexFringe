...
class PrefSpSvSketch{
  private:
    ...
    std::map<int, int> prefixes_to_counts;
    inline static std::map<int, int> symbol_to_mapping;

    std::vector<int> prefix_to_ints(tail* prefix) noexcept;
    void store_prefix(const int pref);

    /**
     * @brief Comparator for sorting the list.
     */
    inline static bool compare_frequencies(const std::pair<int, double>& p1, const std::pair<int, double>& p2){
  ...
  public:
    ...
    std::map<int, double> retrieve_frequencies() const;

    std::vector< std::pair<int, double> > get_most_frequent(const int N) const;
...
std::vector<int> PrefSpSvSketch::prefix_to_ints(tail* prefix) noexcept {
  if(prefix->is_final()){
...
void PrefSpSvSketch::store(tail* t) {
  ...
  const std::vector<int> prefixes = prefix_to_ints(t);
 ...
 */
std::map<int, double> PrefSpSvSketch::retrieve_frequencies() const {
 ...
 */
std::vector< std::pair<int, double> > PrefSpSvSketch::get_most_frequent(const int N) const {
  ...
  const int UPPER = std::min(N, int(tmp.size()));
  if(tmp.size()==0) return tmp;

  return std::vector< std::pair<int, double> > (tmp.begin(), tmp.begin()+UPPER);
}
...
bool PrefSpSvSketch::hoeffding(const PrefSpSvSketch& other) const {
  ...
  for (std::map<int,int>::const_iterator it=prefixes_to_counts.cbegin(); it!=prefixes_to_counts.cend(); ++it){
    ...
    const double lhs = std::abs(this_f - other_f);
...
double PrefSpSvSketch::cosine_similarity(const PrefSpSvSketch& other) const {
  ...
  for (std::map<int,int>::const_iterator it=prefixes_to_counts.cbegin(); it!=prefixes_to_counts.cend(); ++it){
    ...
    absolute1 += std::pow(this_f, 2);
    absolute2 += std::pow(other_f, 2);
  ...
  absolute1 = std::sqrt(absolute1);
  absolute2 = std::sqrt(absolute2);
...
double PrefSpSvSketch::compute_prefix_distance(PrefSpSvSketch& other){
  ...
  for (std::map<int,int>::const_iterator it=prefixes_to_counts.cbegin(); it!=prefixes_to_counts.cend(); ++it){
    ...
    if(other_entry == other_sketch.cend()){
      ...
      res = std::max(std::abs(frequency - other_f), res);
    }
...
double PrefSpSvSketch::get_upper_bound_estimate(PrefSpSvSketch& other, const double mu_hat, const double delta, const double R, const double k){
  ...
  const double confidence_interval = std::max(sqrt(1/(2*c1*M) * log(K_k/delta)), pow(pow(16*NU, 2)/(2*c2*M) * log(K_k/delta), double(1.)/double(4.)));
...
inline void PrefSpSvSketch::operator+(PrefSpSvSketch& other){
  std::map<int, int> unified_map(this->prefixes_to_counts);
  ...
  std::vector< std::pair<int, double> > all_frequencies;
  ...
  for(int i=0; i<std::min(K, static_cast<int>(all_frequencies.size())); ++i){
 ...
 */
inline void PrefSpSvSketch::operator-(PrefSpSvSketch& other){
  throw new std::runtime_error("Undoing PrefSvSketches not implemented (yet?)."); // not straightforward to implement, we can't just split becuase we discard information when merging
  this->size -= other.size;
}
