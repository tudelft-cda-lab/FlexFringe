...
class CountMinSketch{
  private:
    ...
    vector< vector<unsigned int> > counts; // counts stored in table

    inline static vector< HashFunction<int> > indexHasher;
    inline static set<int> seenSymbols;
  ...
  public:
    ...
    static double cosineSimilarity(const CountMinSketch& c1, const CountMinSketch& c2, const set<int>& symbols);

    const vector<double> getDistribution() const;
    const vector<double> getDistribution(const set<int>& symbols) const;
    const pair< vector<double>, vector<double> > getPooledDistributions(const CountMinSketch& other) const;

    static bool hoeffding(const CountMinSketch& c1, const CountMinSketch&c2);
    static bool hoeffding(const CountMinSketch& c1, const CountMinSketch&c2, const set<int>& symbols);
...
inline CountMinSketch::CountMinSketch(const int depth, const int width) : depth(depth), width(width) {
  for(int i = 0; i < depth; ++i){
    counts.push_back(vector<unsigned int>(width)); // INVARIANT: the last row is for the termination symbol
  }
  size = 0;
  terminationCounts = 0;
...
inline const unsigned int CountMinSketch::getCount(const int symbol) const{
  ...
  for(int i = 0; i < depth; ++i){
    ...
    minVal = min(minVal, counts[i][hashedIndex]);
  }
 ...
 */
const vector<double> CountMinSketch::getDistribution() const {
  vector<double> res;
...
const vector<double> CountMinSketch::getDistribution(const set<int>& symbols) const {
  vector<double> res;
 ...
 */
const pair< vector<double>, vector<double> > CountMinSketch::getPooledDistributions(const CountMinSketch& other) const {
  vector<double> this_d, other_d;
...
inline double CountMinSketch::cosineSimilarity(const CountMinSketch& c1, const CountMinSketch&c2, const set<int>& symbols){
  assert(c1.width == c2.width && c1.depth == c2.depth);
...
inline bool CountMinSketch::hoeffding(const CountMinSketch& c1, const CountMinSketch&c2, const set<int>& symbols){
  assert(c1.width == c2.width && c1.depth == c2.depth);
...
/* inline bool CountMinSketch::hoeffdingWithPooling(const CountMinSketch& c1, const CountMinSketch&c2, const double alpha, const int statecount, const int symbolcount, const int correction){
  ...
  for(int i = 0; i < c1.depth; ++i){
    ...
    vector<double> pool1(2);
    vector<double> pool2(2);
